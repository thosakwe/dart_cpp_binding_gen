...
class ExtensionBuilder implements Builder {
  ...
  Future build(BuildStep buildStep) async {
    ...
    resolveName.signature.parameters.addAll([
      new c.Parameter(dartHandleType, 'name'),
    ...
    var initFn = new c.CFunction(new c.FunctionSignature(
        dartHandleType.prefix('DART_EXPORT'), initRc.snakeCase + '_Init'));
    headerFile.body.add(initFn.signature);
    initFn.signature.parameters
        .add(new c.Parameter(dartHandleType, 'parent_library'));
    initFn.body.addAll('''
...
return Dart_Null();
    ...
    resolveName.body.addAll([
      ...
      new c.Code('Dart_Handle stringHandle = (Dart_StringToCString(name, &cname));'),
      new c.Code('if (Dart_IsError(stringHandle)) Dart_PropagateError(stringHandle);'),
    ]);

    natives.forEach((name, body) {
      var literal = body.stringLiteral.stringValue;
      var rc = new ReCase(buildStep.inputId.package + '_extension_' + name);
      resolveName.body.add(new c.Code(
          'if (strcmp("$literal", cname) == 0) result = ${rc.snakeCase};'));

      // Lastly, generate a stub for each function.
      headerFile.body.add(new c.FunctionSignature(c.CType.void$, rc.snakeCase)
        ..parameters.add(
    ...
    buildStep.writeAsString(
        buildStep.inputId.changeExtension('.h'), headerBuf.toString());
    buildStep.writeAsString(
        buildStep.inputId.changeExtension('.cc'), helperBuf.toString());
  }
}
